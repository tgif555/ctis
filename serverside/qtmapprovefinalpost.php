<?php
	...
	if ($action_post != "") { //post มาจาก form approve sptdmnt.php
		...
		$action_approve = md5("qtm_final_approve".$user_login);
		if ($action_post == $action_approve) { //post มาจาก form approve sptdmnt.php
			...
			$sql_qtm = "SELECT * from qtm_mstr where qtm_nbr = '$qtm_nbr' and (qtm_step_code = '40')";
			...
			if ($r_qtm) {
				...
				$qtm_submit_by = $r_qtm['qtm_submit_final_by'];
		...
		if ($qtm_auth_code!="" && $qtm_nbr!="" && $qtm_approve_select!="") {
			if (inlist("35,90,890",$qtm_approve_select)) {
	...
	if ($allow_post) {
		if ($qtm_curr_step == "40") {
			...
			if ($qtm_next_step == "35") { //Return for Revise
				$qtm_next_curprocessor = $qtm_submit_by;
				$qtm_next_curprocessor_email = $qtm_submit_by_email;
				$qtap_text = "Return for Revised";
			}
			elseif ($qtm_next_step == "90") { //Approved
				//ADD WHO CAN READ
				...
				if ($gbv_qt_final_approver1 !="" && !is_null($gbv_qt_final_approver1)) {
					if(!inlist($qtm_whocanread,$gbv_qt_final_approver1)) {
						if ($qtm_whocanread != "") { $qtm_whocanread = $qtm_whocanread .","; }
						$qtm_whocanread = $qtm_whocanread . $gbv_qt_final_approver1;
					}
				}
				if ($gbv_qt_final_approver2 !="" && !is_null($gbv_qt_final_approver2)) {
					if(!inlist($qtm_whocanread,$gbv_qt_final_approver2)) {
						if ($qtm_whocanread != "") { $qtm_whocanread = $qtm_whocanread .","; }
						$qtm_whocanread = $qtm_whocanread . $gbv_qt_final_approver2;
					}
				}
				//
				//ADD CURRENT PROCESSOR
				$qtm_next_curprocessor = $user_login;
				/*
				...
				*/
				$qtap_text = "Final Approved";
			}
			else {
				$qtm_next_curprocessor = "";
				$qtm_next_curprocessor_email = $qtm_submit_by_email;
				$qtap_text = "Final Rejected";
			}
			...
			" qtm_step_date = '$today'," .
			" qtm_approve_final_by = '$user_login'," .
			" qtm_approve_final_date = '$today'," .
			" qtm_approve_final_cmmt = '$qtm_approve_cmmt'," .
			...
			if ($can_sendmail) {
				if ($qtm_next_step == "35") {
					if ($qtm_next_curprocessor_email !="") {
						//mail next_curprocessor (Step=35 - เจ้าของเอกสาร)
				...
				if ($qtm_next_step == "90") {
					if ($qtm_submit_by !="") {
						...
						$mail_subject = "[CTIS]- Quotation หมายเลข $qtm_nbr ได้รับการอนุมัติขั้นสุดท้ายแล้วค่ะ";
						$mail_message = "เรียน คุณ$qtm_submit_by_name,<br><br>" .
