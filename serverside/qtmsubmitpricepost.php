<?php
	...
	// $action_submit = md5("submit_price_approve".$user_login);
	...
	//Approver1
	$approver1_user_id = strtoupper($gbv_qt_price_approver1);
	...
	//Approver2
	$approver2_user_id = strtoupper($gbv_qt_price_approver2);
	...
	$sql_qtm = "SELECT * from qtm_mstr where qtm_nbr = '" . $qtm_nbr . "' and (qtm_step_code = '0' or qtm_step_code = '10')";
	...
	if (!$allow_post) {
		...
		if ($qtm_curr_step == "0" || $qtm_curr_step == "10") {
			$qtm_whocanread = "ADMIN,$user_login,$approver_all_id";
			$qtm_next_curprocessor = $approver_all_id;
			$qtm_approve_code = md5(gen_uuid());
			$qtm_next_step = "20"; //Submited - Await for approver
			...
			" qtm_step_date= '$today'," .
			" qtm_submit_price_by = '$user_login'," .
			" qtm_submit_price_date= '$today'," .
			...
			if ($approver1_user_id != "") {
				...
				$mail_subject = "[CTIS] - Quotation: $qtm_nbr รอท่านพิจารณาอนุมัติค่ะ";
				$mail_message = "เรียน คุณ$approver1_name,<br>" .
					"มี Quotation หมายเลข: $qtm_nbr ซึ่งขออนุมัติโดยคุณ $qtm_create_by_name รอท่านพิจารณาอนุมัติค่ะ<br>" .
				...
				$filename_in_mail = "Quotation_$qtm_nbr" . ".pdf";
				if ($can_sendmail) {
					if ($mail_to!="") {
			...
			if ($user_inform_last_action) {
				...
				$mail_from_email = "ctis_admin@scg.com";
				$mail_to = $user_email;
				$mail_subject = "[CTIS] - คุณได้ส่ง Quotation หมายเลข $qtm_nbr  ไปขออนุมัติค่ะ";
