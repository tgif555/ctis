<?php
	...
	$sql = "SELECT * FROM custpj_mstr " .
	" WHERE (custpj_code LIKE '%'+?+'%' OR custpj_name LIKE '%'+?+'%') and custpj_code !='DUMMY'";
	...
	$sql = " SELECT c.* FROM (" .
		" SELECT ROW_NUMBER() OVER(ORDER BY custpj_seq,custpj_name) AS RowID,*  FROM dbo.custpj_mstr " .
		" WHERE (custpj_code LIKE '%'+?+'%' OR custpj_name LIKE '%'+?+'%') and custpj_code !='DUMMY'" .
	") AS c" .
...
<body onblur="javascript:window.close();">  	
	<form name="form" autocomplete=OFF method="post" action="getcustomer_project.php">
		...
		<table class="table table-bordered table-hover">		
			<tr>
				<td colspan=2>
					<input name="txtKeyword" type="text" id="txtKeyword" value="<?php echo $strKeyword;?>" style="margin:auto" placeholder="* รหัสหรือชื่อลูกค้า">
			...
			while($row = sqlsrv_fetch_array($query, SQLSRV_FETCH_ASSOC)) {
				$custpj_code = html_escape($row["custpj_code"]);
				$custpj_name = html_escape($row["custpj_name"]);
			?>
				<tr>
					<td width=50%>
						<a href="#" OnClick="Javascript:updateOpener('<?php echo $formname?>','<?php echo $opennerfield_code?>','<?php echo $opennerfield_code2?>','<?php echo $custpj_code;?>','<?php echo $custpj_name;?>');">
							<?php echo $custpj_code;?>
						</a>
					</td>     
					<td>
						<?php echo $custpj_name;?>
					</td>					
				</tr>
			<?php
			}
