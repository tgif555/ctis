<?php
	...
	$sql = "SELECT * FROM province_mstr " .
	" WHERE province_th_name LIKE '%'+?+'%'";  
	...
	$sql = " SELECT c.* FROM (" .
		" SELECT ROW_NUMBER() OVER(ORDER BY province_seq,province_th_name) AS RowID,*  FROM dbo.province_mstr " .
		" WHERE province_th_name LIKE '%'+?+'%'" .
	") AS c" .
...
<html>
	...
	<script type="text/javascript">
		...
		function updateOpener(formname,opennerfeild_code,name,name2) {	
...
<body onblur="javascript:window.close();">  	
	<form name="form" autocomplete=OFF method="post" action="getprovince_project.php">
		...
		<table class="table table-bordered table-hover">
			<tr>
				<td>
					<input name="txtKeyword" type="text" id="txtKeyword" value="<?php echo $strKeyword;?>" style="margin:auto" placeholder="* ชื่อจังหวัด">
			...
			while($row = sqlsrv_fetch_array($query, SQLSRV_FETCH_ASSOC)) {
				$province_id = html_escape($row["province_id"]);
				$province_th_name = html_escape($row["province_th_name"]);
			?>
				<tr>
					<td width=40%>
						<a href="#" OnClick="Javascript:updateOpener('<?php echo $formname?>','<?php echo $opennerfield_code?>','<?php echo $province_th_name;?>');">
		...
		echo "<center>";
		if($prev_page) {
			echo " <a href='$_SERVER[SCRIPT_NAME]?page =$prev_page&v=$strKeyword&formname=$formname&opennerfield_code=$opennerfield_code'><button type='button' class='btn btn-default btn-sm' style='font-size:8pt'>Prev</button></a> ";
		}
		for($i=1; $i <= $num_pages; $i++){
			$page1 = $page-2;
			$page2 = $page+2;
			if($i != $page && $i >= $page1 && $i <= $page2) {
				echo "<a href='$_SERVER[SCRIPT_NAME]?page=$i&v=$strKeyword&formname=$formname&opennerfield_code=$opennerfield_code'><button type='button' class='btn btn-primary btn-sm' style='font-size:8pt'>$i</button></a> ";
			}
			elseif($i == $page) {
		...
		if($page != $num_pages) {  
			echo " <a href ='$_SERVER[SCRIPT_NAME]?page=$num_pages&v=$strKeyword&formname=$formname&opennerfield_code=$opennerfield_code'><button type='button' class='btn btn-default btn-sm' style='font-size:8pt'>Last</button</a> ";
		}
		echo "</center>";
		?>
    </form>   
