...
if ($action == "REPORT" && $allow_report) {
	...
	$objPHPExcel->setActiveSheetIndex(0)
				...
				->setCellValue('E1', 'การขึ้นสินค้า')
				->setCellValue('F1', 'หมายเลขใบส่งของ')
				->setCellValue('G1', 'วันที่ส่งของ')
				->setCellValue('H1', 'วันที่รับของ')
				->setCellValue('I1', 'บริษัทขนส่ง')
				->setCellValue('J1', 'หมายเลขอ้างอิง')
				->setCellValue('K1', 'ทะเบียนรถ')
				->setCellValue('L1', 'รหัสสินค้า')
				->setCellValue('M1', 'ชื่อสินค้า-ไทย')
				->setCellValue('N1', 'ชื่อสินค้า-อังกฤษ')
				->setCellValue('O1', 'จำนวนที่ส่ง')
				->setCellValue('P1', 'หน่วย')
				->setCellValue('Q1', 'หมายเลขใบเบิก')
				->setCellValue('R1', 'ประเภทใบเบิก')
				->setCellValue('S1', 'NPD Brand')
				->setCellValue('T1', 'NPD Set No')
				->setCellValue('U1', 'อ้างอิงใบเบิก')
				->setCellValue('V1', 'ชื่อผู้ขอเบิก')
				->setCellValue('W1', 'ชื่อลูกค้า')
				->setCellValue('X1', 'อำเภอ')
				->setCellValue('Y1', 'จังหวัด')
				->setCellValue('Z1', 'วิธีการจัดสส่ง')
				->setCellValue('AA1', 'วัตถุประสงค์ของการเบิก')
				->setCellValue('AB1', 'วันที่ขอเบิก')
				->setCellValue('AC1', 'วันที่ขอรับ')
				->setCellValue('AD1', 'วันที่อนุมัติ')
				->setCellValue('AE1', 'สถานะใบเบิก');
	$excel_row = 2;
	$sql_dlvd = "SELECT * from dlvd_det WITH (NOLOCK)".
		" INNER JOIN dlvm_mstr WITH (NOLOCK) ON dlvm_nbr = dlvd_dlvm_nbr".
		...
		" INNER JOIN material WITH (NOLOCK) ON mat_code = dlvd_mat_code".
	...
	while($r_dlvd = sqlsrv_fetch_array($result_dlvd, SQLSRV_FETCH_ASSOC)) {
		...
		$dlvd_mat_th_name = $r_dlvd['mat_th_name'];
		$dlvd_mat_en_name = $r_dlvd['mat_en_name'];
		if ($dlvd_mat_code == "BC") {
			$dlvd_mat_th_name = findsqlval("sptd_det","sptd_remark","sptd_id",$r_dlvd['dlvd_sptd_id'],$conn);
			$dlvd_mat_en_name = "";
		}
		...
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('E'. $excel_row, $dlvm_wh_status_text,PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('F'. $excel_row, $dlvm_ivm_nbr,PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('G'. $excel_row, $dlvm_ivm_print_date,PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('H'. $excel_row, $dlvm_receive_date,PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('I'. $excel_row, $dlvm_transport_tspm_name,PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('J'. $excel_row, $dlvm_transport_ref_no,PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('K'. $excel_row, $dlvm_transport_car_nbr,PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('L'. $excel_row, $dlvd_mat_code,PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('M'. $excel_row, $dlvd_mat_th_name,PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('N'. $excel_row, $dlvd_mat_en_name,PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('O'. $excel_row, $dlvd_qty,PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('P'. $excel_row, $dlvd_unit_name,PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('Q'. $excel_row, $sptm_nbr,PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('R'. $excel_row, $sptm_type,PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('S'. $excel_row, $sptm_npd_brand_name,PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('T'. $excel_row, $sptm_npd_setno,PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('U'. $excel_row, $sptm_copy_refer,PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('V'. $excel_row, $sptm_req_by_name,PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('W'. $excel_row, $sptm_cust_name,PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('X'. $excel_row, $sptm_customer_amphur,PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('Y'. $excel_row, $sptm_customer_province,PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('Z'. $excel_row, $sptm_delivery_mth_name,PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('AA'. $excel_row, $sptm_reason_name,PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('AB'. $excel_row, dmytx($sptm_req_date),PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('AC'. $excel_row, dmytx($sptm_expect_receipt_date),PHPExcel_Cell_DataType::TYPE_STRING);	
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('AD'. $excel_row, date_format($sptm_approve_date,'d/m/Y'),PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit('AE'. $excel_row, $sptm_step_name,PHPExcel_Cell_DataType::TYPE_STRING);
		$excel_row++;
	}
	...
	$strfilename = "dlvm_report01_det_".$curdate."_".rand()."-".$in_dlvmrpt_req_date1."-".$in_dlvmrpt_req_date2.".xlsx";	
