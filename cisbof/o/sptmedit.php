  ...
  <head>
	...
	<script language="javascript">	
		...
		function setdefaultampv(v) {
			if (v == 'M002' || v == 'M005') {
				frm_sptmedit.sptm_customer_amphur.value='บางซื่อ';
				frm_sptmedit.sptm_customer_province.value='กรุงเทพมหานคร';
			}
			else {
				var customer_number = frm_sptmedit.sptm_customer_number.value;
				var xhttp = new XMLHttpRequest();
				xhttp.onreadystatechange = function() {								
					if (xhttp.readyState == 4 && xhttp.status == 200) {
						var res = xhttp.responseText.split("|")	;
						frm_sptmedit.sptm_customer_amphur.value = res[0];
						frm_sptmedit.sptm_customer_province.value = res[1];								
					}
				}
												...
												<table class="table-bordered" border=0 width="100%" cellpadding=3 cellspacing=0>
													...
													<tr>
														...
														<td>
															...
															<button type="button" name="sptm_customer_help" id="sptm_customer_help" class="btn btn-default" style="vertical-align: top;height:25px;margin:auto" OnClick="helppopup_customer('../_help/getcustampv.php','','','',document.all.sptm_customer_number.value)" data-dismiss="modal">
														...
														<td>
															<select name="sptm_delivery_mth" class="inputtext_s" style="width: 260px;" onchange="setdefaultampv(this.value)">
																...
																while($r_delivery_list=sqlsrv_fetch_array($result_delivery_list, SQLSRV_FETCH_ASSOC)) {
																	...
																	?>
																	<option value="<?php echo $delivery_code;?>" <?php if($sptm_delivery_mth == $delivery_code) {echo "selected";}?>><?php echo $delivery_name;?></option>
																	<?php
																}
																?>
															</select>
													...
													<tr>
														...
														<td>
															<textarea name="sptm_delivery_mth_desc" rows=4 style='width:250px;color:blue' class="inputtext_s" maxlength=255 placeholder="* ระบุหมายเหตุการจัดส่ง"><?php echo $sptm_delivery_mth_desc;?></textarea>
														</td>							
														<td></td>
													</tr>
													<tr>
