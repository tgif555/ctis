...
<table width="100%" border="0" cellspacing="0" cellpadding="4" class="table table-bordered table-condensed">			
	...
	$sql = "SELECT * FROM sptd_det" .
		...
		while($r_sptbs = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC)) {	
			...
			$sptd_dn_nbr = $r_sptbs['sptd_dn_nbr'];
			$sptd_oper_note = $r_sptbs['sptd_oper_note'];
			...
			$sptd_send_manual_foc = $r_sptbs['sptd_send_manual_foc'];
			$manual_foc_color = "orange";
			if ($sptd_send_manual_foc) {
				$manual_foc_color = "green";
			}
			$sptd_dn_status = $r_sptbs['sptd_dn_status'];
			$dn_status_color = "";
			...
			<tr id="trsptd_<?php echo $sptd_id?>">
				...
				<td style="max-width:160px;overflow:hidden;background-color: aliceblue" title="<?php echo $sptd_remark?>">
				...
				<?php if ($sptd_id != $packing_sptd_id) {?>
					...
					<td style="background-color: aliceblue;text-align:right">
						...
						<?php if($can_packing and $sptd_qty_await > 0) {?>
							<a href="sptdmnt.php?sptmnumber=<?php echo encrypt($sptm_nbr, $key)?>&packing_sptd_id=<?php echo $sptd_id?>&actiondetail=packing_bs#trsptd_<?php echo $sptd_id?>">Picking</a>
						<?php }?>
