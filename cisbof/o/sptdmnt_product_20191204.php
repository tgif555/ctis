...
<table border="0" width="100%" cellspacing="0" cellpadding="4" class="table table-bordered table-condensed">			
	...
	while($r_sptd = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC)) {	
		...
		<tr>
			...
			<td style="background-color: lightcyan"><?php echo $sptd_mat_code; ?></td>
			<td style="background-color: lightcyan"><?php echo $sptd_mat_name; ?></td>
			<td style="max-width:140px;overflow:hidden;background-color: lightcyan" title="<?php echo $sptd_remark?>"><?php echo $sptd_remark; ?></td>
			...
			<?php if ($sptd_id != $packing_sptd_id) {?>
				...
				<td style="background-color: lightcyan;text-align:right">
					...
					<?php if($can_packing and $sptd_qty_await > 0) {?>
						<a href="sptdmnt.php?sptmnumber=<?php echo encrypt($sptm_nbr, $key)?>&packing_sptd_id=<?php echo $sptd_id?>&actiondetail=packing_product#product_section">Packing</a>
					<?php }?>
			...
			<?php if ($sptd_id == $packing_sptd_id and $actiondetail=="packing_product") {?>
				...
				<td style="text-align:center">												
					...
					<div class="btn btn-mini btn-danger" style="margin-top:0px; margin-bottom:10px; width: 40px;" onclick="window.location.href='sptdmnt.php?sptmnumber=<?php echo encrypt($sptm_nbr, $key)?>&activeid=<?php echo encrypt($sptd_id,$key)?>#product_section'">
