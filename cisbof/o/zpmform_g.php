<?php
	...
	$header =
		...
		"<table width=100% cellpadding=5 cellspacing=5 style='border-collapse: collapse;border: 1px dotted black; font-size:8pt'>" .
			"<tr>" .
				...
				"<td style='border: 1px dotted black;width:150px;text-align:center'><b>ลูกค้า</b></td>".
				"<td style='border: 1px dotted black;width:150px;text-align:center'><b>อำเภอ/จังหวัด</b></td>".
				"<td style='border: 1px dotted black;width:60px;text-align:center'><b>น้ำหนัก<br>(KG)</b></td>".
				"<td style='border: 1px dotted black;width:100px;text-align:center'><b>วิธีการจัดส่ง</b></td>".
				"<td style='border: 1px dotted black;width:150px;text-align:center'><b>หหมายเหตุการจัดส่ง</b></td>".
			"</tr>".
		"</table>";
	...
	while($r_dlvm = sqlsrv_fetch_array($result_dlvm, SQLSRV_FETCH_ASSOC)) {	
		...
		$dlvm_sptm_delivery_mth = $r_dlvm['sptm_delivery_mth'];
		$dlvm_sptm_delivery_mth_name = findsqlval("delivery_mth","delivery_name","delivery_code",$dlvm_sptm_delivery_mth,$conn);
		...
		$sptm_cust_code = $r_dlvm['sptm_customer_number'];
		$sptm_cust_dummy = html_quot($r_dlvm['sptm_customer_dummy']);
		$sptm_cust_amphur =  html_quot($r_dlvm['sptm_customer_amphur']);
		$sptm_cust_province = html_quot($r_dlvm['sptm_customer_province']);
		...
		$data = "<tr>" .
				...
				"<td style='border: 1px dotted black;width:150px;'>$sptm_cust_name $dlvm_packing_location</td>".
				"<td style='border: 1px dotted black;width:150px;'>$sptm_cust_amphur/$sptm_cust_province</td>".
				"<td style='border: 1px dotted black;width:60px;text-align:center'>$dlvm_packing_weight</td>".
				"<td style='border: 1px dotted black;width:100px;'>$dlvm_sptm_delivery_mth_name</td>".
				"<td style='border: 1px dotted black;width:150px;'>$dlvm_sptm_delivery_mth_desc</td>".
				"</tr>";
		if ($n == $dlvm_nbr_count) {
